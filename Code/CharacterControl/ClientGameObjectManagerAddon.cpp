...
SoldierNPC *ClientGameObjectManagerAddon::getSoldierNPC(const char *gameObjectId)
{
	PE::Handle* pHC = m_components.getFirstPtr();
	...
	for (PrimitiveTypes::UInt32 i = 0; i < m_components.m_size; i++, pHC++) // fast array traversal (increasing ptr)
	{
		...
		{
			SoldierNPC* sNPC = (SoldierNPC*)(pC);
			if (StringOps::strcmp(sNPC->m_gameObjectId, gameObjectId) == 0)
			{
				// equal strings, found our waypoint
				return sNPC;
			}
