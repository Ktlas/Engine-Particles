...
int Event_CreateSoldierNPC::l_Construct(lua_State* luaVM)
{
	...
	const char* gameObjectId = NULL;

	if (!lua_isnil(luaVM, -numArgs))
	{
		// have game object id for reference
		gameObjectId = lua_tostring(luaVM, -numArgs--);
	}
	else
		// ignore
		numArgs--;
	...
	if (!lua_isnil(luaVM, -numArgs))
	{
		// have target game object id
		target = lua_tostring(luaVM, -numArgs--);
	}
	else
		// ignore
		numArgs--;
	...
	StringOps::writeToString(gameObjectId, pEvt->m_gameObjectId, 32);
	StringOps::writeToString(target, pEvt->m_target, 32);
...
//PE_IMPLEMENT_CLASS1(Event_CreateParticlesInstance, Event);
//
//void Event_CreateParticlesInstance::SetLuaFunctions(PE::Components::LuaEnvironment* pLuaEnv, lua_State* luaVM)
//{
//	static const struct luaL_Reg l_Event_CreateParticlesInstance[] = {
//		{"Construct", l_Construct},
//		{NULL, NULL} // sentinel
//	};
//
//	// register the functions in current lua table which is the table for Event_CreateParticlesInstance
//	luaL_register(luaVM, 0, l_Event_CreateParticlesInstance);
//}
//
//int Event_CreateParticlesInstance::l_Construct(lua_State* luaVM)
//{
//	PE::Handle h("EVENT", sizeof(Event_CreateParticlesInstance));
//
//	// get arguments from stack
//	int numArgs, numArgsConst;
//	numArgs = numArgsConst = 21;
...
//
//	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
//	Event_CreateParticlesInstance* pEvt = new(h) Event_CreateParticlesInstance(pContext->m_gameThreadThreadOwnershipMask);
//
//	const char* name = lua_tostring(luaVM, -numArgs--);
//	const char* package = lua_tostring(luaVM, -numArgs--);
//
