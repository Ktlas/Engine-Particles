...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	const char* optionalNextWayPointName1 = lua_tostring(luaVM, -numArgs--);
	const char* optionalNextWayPointName2 = lua_tostring(luaVM, -numArgs--);

	int needToRun = (int)lua_tonumber(luaVM, -numArgs--);
	pEvt->m_needToRunToThisWaypoint = needToRun;

	int randomizeNextWaypoint = (int)lua_tonumber(luaVM, -numArgs--);
	pEvt->m_randomizeNextWaypoint = randomizeNextWaypoint;
	...
	StringOps::writeToString(optionalNextWayPointName1, pEvt->m_optionalNextWayPointName1, 32);
	StringOps::writeToString(optionalNextWayPointName2, pEvt->m_optionalNextWayPointName2, 32);
...
: Component(context, arena, hMyself)
{
	...
	StringOps::writeToString(pEvt->m_optionalNextWayPointName1, m_optionalNextWayPointName1, 32);
	StringOps::writeToString(pEvt->m_optionalNextWayPointName2, m_optionalNextWayPointName2, 32);

	m_needToRunToThisWaypoint = pEvt->m_needToRunToThisWaypoint;
	m_randomizeNextWaypoint = pEvt->m_randomizeNextWaypoint;
