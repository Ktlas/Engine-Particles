...
void DebugRenderer::createQuadMesh(Vector3 lineColor, Vector3 point1, Vector3 point2) 
{
	Matrix4x4 m;
	m.loadIdentity();
	Vector3 linepts[4];
...
void DebugRenderer::createBoundingVolumeMesh(Vector3 mins, Vector3 maxs) {
	if (EnableDebugRendering && m_numAvailableLineLists)
	{
		Matrix4x4 m;
		m.loadIdentity();
