...
#include "PrimeEngine/Scene/ParticlesSystemSceneNode.h"
#include "PrimeEngine/Scene/ParticlesSystem.h"
...
namespace PE {
	namespace Components {
		...
		PE_IMPLEMENT_CLASS1(ParticlesManager, SceneNode);

		// Static member variables
		Handle ParticlesManager::s_myHandle;

		// Singleton ------------------------------------------------------------------

		void ParticlesManager::Construct(PE::GameContext& context, PE::MemoryArena arena)
		{
			Handle handle("ParticlesManager", sizeof(ParticlesManager));
			ParticlesManager* pParticlesManager = new(handle) ParticlesManager(context, arena, handle);
			pParticlesManager->addDefaultComponents();
			// Singleton
		...
		// Constructor -------------------------------------------------------------
		ParticlesManager::ParticlesManager(PE::GameContext& context, PE::MemoryArena arena, Handle hMyself)
			...
			, m_numFreeing(0)
		{
			...
			m_numAvaialble = NUM_ParticlesSceneNodes;
			for (int i = 0; i < NUM_ParticlesSceneNodes; i++)
				m_hAvailableSNs[i] = i;
		...
		// Methods      ------------------------------------------------------------
		void ParticlesManager::addDefaultComponents()
		{
			Component::addDefaultComponents();

			PE_REGISTER_EVENT_HANDLER(Events::Event_PRE_GATHER_DRAWCALLS, ParticlesManager::do_PRE_GATHER_DRAWCALLS);
		...
		//void ParticlesManager::createRootLineMesh()
		//{
		...
		//	ParticlesManager::Instance()->createLineMesh(true, m, &linepts[0].m_x, numPts, 0);// send event while the array is on the stack
		//}

		//void ParticlesManager::createLineMesh(bool hasTransform, const Matrix4x4& transform, float* pRawData, int numInRawData, float timeToLive, float scale /* = 1.0f*/)
		//{
		...
		//void ParticlesManager::createBoundingVolumeMesh(Vector3 mins, Vector3 maxs) {
		//	if (EnableDebugRendering && m_numAvailableLineLists)
		//	{
		//		Matrix4x4 m;
		//		m.loadIdentity();
		...
		//		linepts[0] = mins;
		//		linepts[2] = mins;
		//		linepts[2].m_x = maxs.getX();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		//		linepts[2].m_x = mins.getX();
		//		linepts[2].m_y = maxs.getY();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		//		linepts[2].m_y = mins.getY();
		//		linepts[2].m_z = maxs.getZ();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		//		linepts[2].m_z = mins.getZ();
		//		linepts[0].m_y = maxs.getY();
		//		linepts[2].m_y = maxs.getY();
		//		linepts[2].m_x = maxs.getX();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		//		linepts[2].m_x = mins.getX();
		//		linepts[2].m_z = maxs.getZ();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		//		linepts[0].m_y = mins.getY();
		//		linepts[0].m_x = maxs.getX();
		//		linepts[2].m_x = maxs.getX();
		//		linepts[2].m_z = mins.getZ();
		//		DebugRenderer::Instance()->createLineMesh(false, m, &linepts[0].m_x, 2, 0);
		...
		//void ParticlesManager::createTextMesh(const char* str, bool isOverlay2D, bool is3D, bool is3DFacedToCamera, bool is3DFacedToCameraLockedYAxis, float timeToLive, Vector3 pos, float scale, int& threadOwnershipMask)
		//{
		...
		void ParticlesManager::createParticlesSpawner(const char* str, bool isOverlay2D, bool is3D, bool is3DFacedToCamera, bool is3DFacedToCameraLockedYAxis, float timeToLive, Vector3 pos, float scale, int& threadOwnershipMask)
		{
			if (EnableDebugRendering && m_numAvaialble)
			{
				int index = m_hAvailableSNs[--m_numAvaialble];
				Handle& h = m_hSNPool[index];
				ParticlesSystemSceneNode* pPSSN = 0;
				if (h.isValid())
				{
					//scene node has already been created
					pPSSN = h.getObject<ParticlesSystemSceneNode>();
					assert(pPSSN->isEnabled() == false); // this SN should never be in the available list if it is enabled
					pPSSN->setSelfAndMeshAssetEnabled(true);
				}
				else
				{
					h = PE::Handle("PARTICLES_SYSTEM_SCENE_NODE", sizeof(ParticlesSystemSceneNode));
					pPSSN = new(h) ParticlesSystemSceneNode(*m_pContext, m_arena, h);
					pPSSN->addDefaultComponents();
					addComponent(h);
				}
				m_lifetimes[index] = timeToLive;
				ParticlesSystemSceneNode::DrawType drawType = ParticlesSystemSceneNode::InWorld;
				if (isOverlay2D)
				{
					drawType = ParticlesSystemSceneNode::Overlay2D;
				...
				if (is3DFacedToCamera)
					drawType = ParticlesSystemSceneNode::Overlay2D_3DPos;
					//drawType = ParticlesSystemSceneNode::InWorldFacingCamera;
				pPSSN->loadTexture_needsRC(drawType, threadOwnershipMask);
				pPSSN->m_base.setPos(pos);
				pPSSN->m_scale = scale;
			}
		}


		void ParticlesManager::do_PRE_GATHER_DRAWCALLS(Events::Event* pEvt)
		{
			...
			for (int i = 0; i < NUM_ParticlesSceneNodes; i++)
			{
				...
				if (h.isValid())
				{
					ParticlesSystemSceneNode* pPSSN = h.getObject<ParticlesSystemSceneNode>();
					if (pPSSN->isEnabled())
					{
						if (m_lifetimes[i] < 0.0f)
						{
							pPSSN->setSelfAndMeshAssetEnabled(false);
		...
		void ParticlesManager::postPreDraw(int& threadOwnershipMask)
		{
