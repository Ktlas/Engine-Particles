#ifndef __PYENGINE_2_0_PARTICLESMANAGER_H__
#define __PYENGINE_2_0_PARTICLESMANAGER_H__
...
namespace PE {
	namespace Components {
		...
		struct ParticlesManager : public SceneNode
		{
			PE_DECLARE_CLASS(ParticlesManager);
			// Singleton ------------------------------------------------------------------
			...
			{
				return s_myHandle.getObject<ParticlesManager>();
			}
			...
			{
				// Singleton
				ParticlesManager::s_myHandle = handle;
			}

			// Constructor -------------------------------------------------------------
			ParticlesManager(PE::GameContext& context, PE::MemoryArena arena, Handle hMyself);
			...
			void createParticlesSpawner(const char* str, bool isOverlay2D, bool is3D, bool is3DFacedToCamera,
				bool is3DFacedToCameraLockedYAxis, float timeToLive, Vector3 pos, float scale,
				int& threadOwnershipMask);
		...
		private:
			...
			static Handle s_myHandle;
			Handle m_hMyParticlesMesh;
			static const int NUM_ParticlesSceneNodes = 64;
			Handle m_hSNPool[NUM_ParticlesSceneNodes];
			int m_hFreeingSNs[NUM_ParticlesSceneNodes];
			int m_hAvailableSNs[NUM_ParticlesSceneNodes];
			float m_lifetimes[NUM_ParticlesSceneNodes];
			int m_numAvaialble;
			int m_numFreeing;
