...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (false && pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		//pCamSN->do_CALCULATE_TRANSFORMATIONS(pEvt);
		// BoundingBox debug rendering
		for (int i = 0; i < pMeshCaller->m_instances.m_size; ++i)
		{
			MeshInstance* pMeshInst = pMeshCaller->m_instances[i].getObject<MeshInstance>();
			PE::Handle hMeshSceneNode = pMeshInst->getFirstParentByType<SceneNode>();
			Matrix4x4 meshTrans;
			//meshTrans.loadIdentity();
			//hMeshSceneNode.getObject<PE::Components::SceneNode>()->do_CALCULATE_TRANSFORMATIONS(pEvt);
			if (hMeshSceneNode.isValid())
			{
				meshTrans = hMeshSceneNode.getObject<SceneNode>()->m_worldTransform;
				//Vector3 testPos = hMeshSceneNode.getObject<SceneNode>()->m_worldTransform.getPos();
			}
			else 
			{
				SceneNode* pSN = pMeshInst->getFirstComponentHandle<SceneNode>().getObject<SceneNode>();
				//pMeshCaller->getFirstParentByType
				meshTrans = pSN->m_worldTransform;
				//meshTrans = pMeshInst->getFirstComponent<SceneNode>()->m_worldTransform;
			}
			Vector3 mins = meshTrans * pMeshCaller->m_axisAlignedBoundingBox.m_Min;
			Vector3 maxs = meshTrans * pMeshCaller->m_axisAlignedBoundingBox.m_Max;

			if (pCamSN->checkAABBIntersectsFrustum(mins, maxs)) 
			{
				//PEINFO("Rendered \n");
				pMeshInst->m_culledOut = false;
				++pMeshCaller->m_numVisibleInstances;
				// Draw aabb in the scene for debug purpose
				DebugRenderer::Instance()->createBoundingVolumeMesh(mins, maxs);
			}
			else 
			{
				pMeshInst->m_culledOut = true;
				//PEINFO("Culled Out \n");
			}
		}
